...
def mdsuite_project():
    ...
    C60 Fullerene test case to compute neighbors.
...
def test_analysis(mdsuite_project):
    ...
    Test the following:
        ...
        * Extraction of neighbors per atom.

    """

    computation = mdsuite_project.run.FindNeighbors(
        max_bond_length=1.5, number_of_configurations=2, plot=True
    )

    assert computation['C60'].data_dict['System'] == MutableDict({'3': [60, 60]})
