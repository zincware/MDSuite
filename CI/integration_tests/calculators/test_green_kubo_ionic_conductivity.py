...
Description: Test for the green_kubo_ionic_conductivity
"""
...
def true_values() -> dict:
    ...
    data = static_path / 'green_kubo_ionic_conductivity.json'
...
def test_project(traj_files, true_values, tmp_path):
    ...
    key = project.run.GreenKuboIonicConductivity.result_keys
    computation = project.run.GreenKuboIonicConductivity()

    data_dict = computation["NaCl"].data_dict["System"]

    np.testing.assert_array_almost_equal(data_dict[key[0]], true_values['x'])
    np.testing.assert_array_almost_equal(data_dict[key[1]], true_values['uncertainty'])


# def test_gkic_experiment(traj_files, true_values, tmp_path):
#     """Test the green_kubo_ionic_conductivity called from the experiment class
#     """
#     os.chdir(tmp_path)
#     project = mds.Project()
#     project.add_experiment("NaCl", data=traj_files[0], timestep=0.002, temperature=1400)
