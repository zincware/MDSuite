...
Construction: the integrated heat current ``Q(t) = integral_0^t J(s) ds``
is fed as a three-dimensional Wiener process with prescribed diffusion
constant ``D_Q`` per Cartesian component. The mean-square displacement
obeys ``<|Q(t) - Q(0)|**2> = 6 D_Q t``, and the Einstein-Helfand formula
gives

    kappa = (1 / (6 V kB T**2)) lim_t d/dt <|Q(t) - Q(0)|**2>
          = D_Q / (V kB T**2).

The calculator's SI prefactor (``1 / (V T kB)`` with the temperature**2
denominator handled through the gradient interpretation) reproduces this.

Replaces ``_test_einstein_helfand_thermal_conductivity.py`` (disabled
snapshot test).
"""
import os
...
def test_calculator(tmp_path, desired_memory):
    """End-to-end correctness of the Einstein-Helfand thermal conductivity."""
    with mds.utils.helpers.change_memory_fraction(desired_memory=desired_memory):
        time_step = 0.1
        temperature = 300.0
        box_l = [5.0, 5.0, 5.0]
        # Set the per-component diffusion so the analytic kappa
        # ``D_Q / (V kB T)`` is order unity — keeps the MSD values in a
        # numerical range where ``fit_einstein_curve`` (UnivariateSpline +
        # linear regression) behaves well.
        ...
        Q = _wiener_3d(n_step=n_step, dt=time_step, diffusion=D_Q, seed=20260525)
        ...
        moment_prop = mdsuite_properties.integrated_heat_current
        ...
        exp.add_data(ScriptInput(data=data, metadata=metadata, name="eh_kappa_synth"))

        result = exp.run.EinsteinHelfandThermalConductivity(
            plot=False,
        ...
        # The calculator's SI prefactor is 1 / (V T kB), and the analytic
        # MSD is 6 D_Q t (3D Brownian motion summed over components).
        ...
        np.testing.assert_allclose(
            ...
            err_msg="E-H thermal-conductivity MSD does not match 6*D_Q*t",
        )

        # kappa = 1/6 * gradient = prefactor * D_Q
