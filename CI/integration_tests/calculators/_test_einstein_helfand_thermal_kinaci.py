...
Description: Test for the einstein_helfand_thermal_kinaci
"""
...
def true_values() -> dict:
    ...
    data = static_path / 'einstein_helfand_thermal_kinaci.json'
...
def test_project(traj_files, true_values, tmp_path):
    ...
    project.run_computation.EinsteinHelfandThermalKinaci(plot=False)

    data_dict = project.load_data.EinsteinHelfandThermalKinaci()[0].data_dict

    data = Path(
        r'C:\Users\fabia\Nextcloud\DATA\JupyterProjects\MDSuite\CI\integration_tests\calculators\data\einstein_helfand_thermal_kinaci.json')
...
def test_experiment(traj_files, true_values, tmp_path):
    ...
    project.experiments['NaCl'].run_computation.EinsteinHelfandThermalKinaci(plot=False)

    data_dict = project.experiments['NaCl'].load_data.EinsteinHelfandThermalKinaci()[0].data_dict
