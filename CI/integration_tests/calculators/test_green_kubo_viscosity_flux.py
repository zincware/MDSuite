...
(stress-flux form). Same OU-process construction as
``test_green_kubo_viscosity.py``; the only difference is that this
calculator's prefactor multiplies by the volume (different stress-vs.
momentum-flux convention), so the analytic answer carries a different
volume factor.

Replaces ``_test_green_kubo_viscosity_flux.py``, which was disabled.
"""
import os
...
def test_calculator(tmp_path, desired_memory):
    """End-to-end correctness of the GK viscosity (stress flux) calculator."""
    with mds.utils.helpers.change_memory_fraction(desired_memory=desired_memory):
        ...
        flux = ornstein_uhlenbeck_3d(
            n_step=n_step, dt=time_step, sigma=sigma, tau=tau, seed=20260522
        )
        ...
        flux_prop = mdsuite_properties.stress_viscosity
        ...
        exp.add_data(ScriptInput(data=data, metadata=metadata, name="eta_flux_synth"))

        result = exp.run.GreenKuboViscosityFlux(
            plot=False,
        ...
        # GreenKuboViscosityFlux prefactor in SI:
        #   numerator   = V
        #   denominator = 3 (N-1) T kB
        # so eta -> V * sigma^2 tau / (T kB) for an OU stress flux.
        volume = float(np.prod(box_l))
        eta_recovered = system["viscosity"][0]
        eta_analytic = sigma**2 * tau * volume / (temperature * units.boltzmann)
