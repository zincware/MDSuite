...
from mdsuite.database.simulation_database import PropertyInfo, SpeciesInfo, TrajectoryChunkData, TrajectoryMetadata
...
def get_species_list(n_species=1, prop_names=['Positions'], n_particles=17, n_dims=3):
    ...
    for i in range(n_species):
        ret.append(SpeciesInfo(name=f'sp_test_{i}', mass=1.1, n_particles=n_particles, properties=props))

    return ret


def test_species_info_equal():
    properties_0 = [PropertyInfo(name='test0', n_dims=2),
                    PropertyInfo(name='test1', n_dims=2)]
    properties_1 = [PropertyInfo(name='test0', n_dims=2),
...
def test_read_script_input():
    ...
    properties = [PropertyInfo(name='Positions', n_dims=n_dims),
