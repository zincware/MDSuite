...
class EinsteinHelfandThermalConductivity(TrajectoryCalculator, ABC):
    ...
    def __init__(
        ...
        """Einstein-Helfand thermal conductivity calculator.
    ...
    def ensemble_operation(self, ensemble):
        """Accumulate the heat-current MSD for one window.

        ``ensemble`` arrives shaped ``(n_particles, n_timesteps, 3)``. The
        reference frame is ``ensemble[:, 0:1, :]`` (singleton time axis so
        broadcasting stays correct even on empty minibatches). Reduce over
        the Cartesian and particles axes to produce a length-``T`` MSD.
        """
