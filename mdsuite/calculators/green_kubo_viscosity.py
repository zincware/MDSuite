...
class GreenKuboViscosity(Calculator):
    ...
    def __init__(self, experiment, plot=False, data_range=500, x_label='Time (s)', y_label=r'SACF ($C^{2}\cdot m^{2}/s^{2}$)',
        ...
        self.tensor_choice = False  # Load tensor_values as a tensor
        ...
        with hf.File(os.path.join(experiment.database_path, 'database_path.hdf5'), "r+") as database:
            ...
            if self.loaded_property not in database:
                ...
                print("Current calculation is finished and stored in the database_path, proceeding with analysis")
    ...
    def _calculate_system_current(self):
        """
        Calculate the ionic current of the experiment

        Parameters
        ----------
        velocity_matrix : np.array
                tensor of experiment velocities for use in the current calculation

        Returns
        -------
        system_current : np.array
                thermal current of the experiment as a vector of shape (number_of_configurations, 3)
        """

        # collect machine properties and determine batch size
        self._collect_machine_properties(group_property='Velocities')
        n_batches = np.floor(self.experiment.number_of_configurations / self.batch_size['Parallel'])
        remainder = int(self.experiment.number_of_configurations % self.batch_size['Parallel'])

        # add a dataset in the database_path and prepare the structure
        database = Database(name=os.path.join(self.experiment.database_path, "database_path.hdf5"), architecture='simulation')
        db_object = database.open()  # open a database_path
        path = join_path(self.loaded_property, self.loaded_property)  # name of the new database_path
        dataset_structure = {path: (self.experiment.number_of_configurations, 3)}
        database.add_dataset(dataset_structure, db_object)  # add a new dataset to the database_path
        ...
        if remainder > 0:
            ...
            stress_tensor = self.experiment.load_matrix('Stress', select_slice=np.s_[:, start:],
        ...
        database.close(db_object)  # close the database_path
        self.experiment.memory_requirements = database.get_memory_information()  # update the memory info in experiment

    def _calculate_viscosity(self):
        """
        Calculate the viscosity of the experiment
        """
        ...
        # convert to SI units.
        prefactor_units = self.experiment.units['pressure'] ** 2 * self.experiment.units['length'] ** 3 * self.experiment.units[
            'time'] / self.experiment.units['energy']
        sigma = prefactor * prefactor_units * np.array(sigma)
        parsed_autocorrelation /= max(parsed_autocorrelation)  # Get the normalized autocorrelation plot tensor_values

        self._update_properties_file(data=[str(np.mean(sigma)), str((np.std(sigma) / np.sqrt(len(sigma))))])

        plt.plot(self.time * self.experiment.units['time'], parsed_autocorrelation)  # Add a plot
