...
class AngularDistributionFunction(Calculator, ABC):
    ...
    def __init__(self, experiment):
        ...
        self.bin_range = [
            0.0,
            3.15,
        ]  # from 0 to pi -. ??? 3.1415 -> 3.15 = failed rounding. I rounding up here, to
    ...
    def run_experimental_analysis(self):
        ...
        if self.use_tf_function:
            ...
            def _get_triplets(x):
                return get_triplets(
                    x,
                    r_cut=self.r_cut,
                    n_atoms=self.experiment.number_of_atoms,
                    ...
                    angles.update(
                        {
                            ...
                            + tf.histogram_fixed_width(
                                get_angles(r_ij_mat, tmp), self.bin_range, self.bins
                            )
                        }
                    )
                else:
        ...
        for species in itertools.combinations_with_replacement(species_indices, 3):
            ...
            if self.save or self.export:
                ...
                log.warning(
                    "Delete duplicates is not supported for calculators that involve more then 3 species!"
                )
