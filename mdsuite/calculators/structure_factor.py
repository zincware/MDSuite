...
class StructureFactor(Calculator):
	...
	def __call__(self, plot=True,  save=True, data_range=1):
		...
		for species in self.experiment.species:
			...
			self.number_of_atoms += self.experiment.species[species][
				'n_particles']

		for species in self.experiment.species:
			self.elements_dict[species] = {}
			self.elements_dict[species]['particle_density'] = \
			...
			self.elements_dict[species]['molar_fraction'] = self.experiment.species[species]['n_particles'] / self.number_of_atoms

		log.info(f'The species dictionary of the experiment class {self.experiment.species}')
	...
	def atomic_form_factors(self, scattering_scalar):
		...
		for el in list(self.experiment.species):
			...
			atomic_form_fac = (
				self.gauss(el_frame.iloc[0, 1], el_frame.iloc[0, 2],
				           scattering_scalar)
				+ self.gauss(el_frame.iloc[0, 3], el_frame.iloc[0, 4],
				             scattering_scalar)
				+ self.gauss(el_frame.iloc[0, 5], el_frame.iloc[0, 6],
				             scattering_scalar)
				+ self.gauss(el_frame.iloc[0, 7], el_frame.iloc[0, 8],
				             scattering_scalar)
				+ el_frame.iloc[0, 9]
			)
			atomic_form_di[el] = {}
	...
	def average_atomic_form_factor(self, scattering_scalar):
		...
		for el in self.experiment.species:
			sum1 += self.elements_dict[el]['molar_fraction'] * atomic_form_facs[el][
				"atomic_form_factor"]
	...
	def partial_structure_factor(self, scattering_scalar, elements):
		...
		# if self.plot_partial_structure_factor:
		# 	plt.figure()
		# 	plt.plot(self.radii, integrand)
		# 	plt.show()
	...
	def total_structure_factor(self, scattering_scalar):
		...
		for rdf_name in self.rdf_dict:
			...
			s_12, _, _ = self.partial_structure_factor(scattering_scalar,
			                                           elements)
			s_in = self.weight_factor(scattering_scalar, elements) * s_12
			if elements[0] != elements[
				1]:  # S_ab and S_ba need to be considered
				elements2 = [elements[1], elements[0]]
				s_21, _, _ = self.partial_structure_factor(scattering_scalar,
				                                           elements2)
	...
	def run_calculator(self):
		...
		# print('trying to load the data', data.data_dict, type(data))
		for key in self.rdf_dict:
		...
		data = {self.result_series_keys[0]: self.Q_arr.tolist(),
